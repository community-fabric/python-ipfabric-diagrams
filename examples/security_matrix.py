...
Takes a list of source IPs/subnets and a list of destination IPs/subnets and for a specific combination of protocol and ports,
tests both the reachability and the applicable security policy, and outputs those results in a Rich table. 
"""
...
def isReachable (instance, src_ip, dst_ip, prot, src_ports='1025-10000', dst_ports='80'):
    ...
    path_to_request=Unicast(startingPoint=src_ip,destinationPoint=dst_ip,protocol=prot,srcPorts=src_ports,dstPorts=dst_ports)
    ...
    return {'forwarding':forwardsOK,'allowed_by_policy':allowed,'failure_description':failure_desc}

def reachabilityMatrix (ipfd,src_list,dst_list,prot,src_ports,dst_ports):
    ...
    with Progress(transient=True) as progress:
        ...
        for source in src_list:
            ...
            for dest in dst_list:
                temp_row.append(isReachable(ipfd,source,dest,prot,src_ports,dst_ports))
...
def printMatrix (reach_matrix,title='Reachability Matrix'):
...
if __name__ == "__main__":
    ...
    ipfd=IPFDiagram(verify=False)
    #ipfd.snapshot_id='snapshot ID for query'
    ...
    result=reachabilityMatrix(ipfd,src_list,dst_list,'tcp','1024-10000','80')
    printMatrix(result,title="Test Matrix")
